import axios from 'axios';
import React, { useEffect, useState } from 'react';
import { useNavigate, useParams } from 'react-router-dom';

function BookStoreUpdate(props) {

  //1. usl경로에서 code값을 추출하여 가져온다
  const {num} = useParams();

  // 2. 상태변수 
  const[form,setForm] = useState({
    name:'',
    area1:'',
    area2:'',
    area3:'',
    book_cnt:0,
    owner_nm:'',
    tel_nm:''
  });

  const navigate = useNavigate();

  useEffect(()=>{
    axios.get(`https://port-0-backend-express-server-mkvwe63p223f9070.sel3.cloudtype.app/bookstore/${num}`)
    // 성공시
    .then(res=>{
      console.log('서버 응답 값 :', res.data);
      setForm(res.data);
    })
    // 실패시 
    .catch(err=> console.log('조회 오류 : ', err));
  },[num]);


  // 2. 폼태그에 입력시 발생되는 함수
const handleChange = (e) => {
  const { name, value } = e.target;
  setForm(prev => ({
    ...prev,
    [name]: name === 'book_cnt' ? Number(value) : value
  }));
};

  // 수정하기 버튼 클릭시 내용 전송을 위한 함수
  const handleSubmit=(e)=>{
    e.preventDefault(); //새로고침 방지

    // 비동기 방식으로 업데이트 할 내용을 백엔드로 전달함
    axios.put(`https://port-0-backend-express-server-mkvwe63p223f9070.sel3.cloudtype.app/bookstoreupdate/${num}`,{
      name:form.name,
      area1:form.area1,
      area2:form.area2,
      area3:form.area3,
      book_cnt:form.book_cnt,
      owner_nm:form.owner_nm,
      tel_nm:form.tel_nm
    })
    .then(()=>{
      alert('상품정보가 수정 완료되었습니다.');
      navigate('/bookstore');//bookstore페이지로 이동
    })
    .catch(//통신이 실패할 경우
      err=> console.log('수정실패 :' ,err))
  }
  return (
    <main>
      <section>
        <h2>교보문구 DB 수정 (업데이트)를 위한 페이지</h2>
        <form onSubmit={handleSubmit}>
          <div>
            <p>
              <label htmlFor="num">Code</label>
              <input type="text" id='num' name='num' value={num} readOnly />
            </p>
          </div>
            <div>
              <label htmlFor="name">서점명</label>
              <input type="text" id='name' name='name' value={form.name} onChange={handleChange} required/>
            </div>
            <div>
              <label htmlFor="area1">지역1(시)</label>
              <select name="area1" id="area1" onChange={handleChange} value={form.area1} required>
                <option value="">지역을 선택해주세요</option>
                <option value="서울">서울</option>
                <option value="경기">경기</option>
                <option value="경남">경남</option>
                <option value="광주">광주</option>
                <option value="강원">강원</option>
                <option value="대전">대전</option>
                <option value="대구">대구</option>
                <option value="부산">부산</option>
                <option value="제주도">제주도</option>
              </select>
            </div>
            <div>
              <label htmlFor="area2">지역2(구)</label>
              <select name="area2" id="area2" onChange={handleChange} value={form.area2} required>
                <option value="">지역을 선택해주세요</option>
                <option value="서초">서초</option>
                <option value="성남">성남</option>
                <option value="남구">남구</option>
                <option value="창원">창원</option>
                <option value="서귀포">서귀포</option>
                <option value="수영">수영</option>
                <option value="경기">경기</option>
              </select>
            </div>
            <div>
              <label htmlFor="area3">지역3(동)</label>
              <select name="area3" id="area3" onChange={handleChange} value={form.area3} required>
                <option value="">지역을 선택해주세요</option>
                <option value="방배">방배</option>
                <option value="구기동">구기동</option>
                <option value="인사동">인사동</option>
                <option value="명동">명동</option>
                <option value="익선동">익선동</option>
                <option value="삼청동">삼청동</option>
              </select>
            </div>
            <div>
              <label htmlFor="book_cnt">상품개수</label>
              <input type="number" id='book_cnt' name='book_cnt' value={form.book_cnt} onChange={handleChange} required/>
            </div>
            <div>
              <label htmlFor="">주문자</label>
              <input type="text" id='owner_nm' name='owner_nm' value={form.owner_nm} onChange={handleChange} required/>
            </div>
            <div>
              <label htmlFor="">전화번호</label>
              <input type="text" id='tel_nm' name='tel_nm' value={form.tel_nm} onChange={handleChange} required/>
            </div>
            <button >등록하기</button>
          </form>
      </section>
    </main>
  );
}


export default BookStoreUpdate;
