import axios from 'axios';
import React, { useEffect, useState,useContext } from 'react';
import { useNavigate } from 'react-router-dom';
import { AlertContext } from './AlertContext';
function BookStoreCreate(props) {
  // 상태변수 선언하기

      const {setBookCount} = useContext(AlertContext);
  const [form, setForm] = useState({
    name:'',
    area1:'',
    area2:'',
    area3:'',
    book_cnt:0,
    owner_nm:'',
    tel_nm:''
  });


    const navigate = useNavigate(); 
  // 사용자가 입력폼에 데이터 입력시 발생되는 함수 
  const handleChange=(e)=>{
    const {name ,value} = e.target;//입력값 변수 

    setForm((prev)=>({
      ...prev,
      [name]:name==='book_cnt'?Number(value):value
    }));
  }

  // 신규 상품 등록하기 버튼 클릭시 내용 백엔드로 주소 전송하기 
const handleSubmit = (e) => {
  e.preventDefault(); // 새로고침 방지
  axios
    .post('http://localhost:9070/bookstore', form)
    .then(() => {
      setBookCount(count => count+1); //숫자 증강
      alert('상품이 등록 완료되었습니다.');
      navigate('/bookstore'); // 목록 페이지로 이동
    })
    .catch((err) => {
      console.error(err);
      alert('등록 중 오류가 발생되었습니다.');
    });
};

  return (
    <main>
      <section>
        <h2>교보문구 DB입력</h2>
          <form onSubmit={handleSubmit}>
            <div>
              <label htmlFor="name">서점명</label>
              <input type="text" id='name' name='name' value={form.name} onChange={handleChange} required/>
            </div>
            <div>
              <label htmlFor="area1">지역1(시)</label>
              <select name="area1" id="area1" onChange={handleChange} required>
                <option value="">지역을 선택해주세요</option>
                <option value="서울">서울</option>
                <option value="경기">경기</option>
                <option value="경남">경남</option>
                <option value="광주">광주</option>
                <option value="강원">강원</option>
                <option value="대전">대전</option>
                <option value="대구">대구</option>
                <option value="부산">부산</option>
                <option value="제주도">제주도</option>
              </select>
            </div>
            <div>
              <label htmlFor="area2">지역2(구)</label>
              <select name="area2" id="area2" onChange={handleChange} required>
                <option value="">지역을 선택해주세요</option>
                <option value="서초">서초</option>
                <option value="성남">성남</option>
                <option value="남구">남구</option>
                <option value="창원">창원</option>
                <option value="서귀포">서귀포</option>
                <option value="수영">수영</option>
                <option value="경기">경기</option>
              </select>
            </div>
            <div>
              <label htmlFor="area3">지역3(동)</label>
              <select name="area3" id="area3" onChange={handleChange} required>
                <option value="">지역을 선택해주세요</option>
                <option value="방배">방배</option>
                <option value="구기동">구기동</option>
                <option value="인사동">인사동</option>
                <option value="명동">명동</option>
                <option value="익선동">익선동</option>
                <option value="삼청동">삼청동</option>
              </select>
            </div>
            <div>
              <label htmlFor="book_cnt">상품개수</label>
              <input type="number" id='book_cnt' name='book_cnt' value={form.book_cnt} onChange={handleChange} required/>
            </div>
            <div>
              <label htmlFor="">주문자</label>
              <input type="text" id='owner_nm' name='owner_nm' value={form.owner_nm} onChange={handleChange} required/>
            </div>
            <div>
              <label htmlFor="">전화번호</label>
              <input type="text" id='tel_nm' name='tel_nm' value={form.tel_nm} onChange={handleChange} required/>
            </div>
            <button >등록하기</button>
          </form>
      </section>
    </main>
  );
}

export default BookStoreCreate;